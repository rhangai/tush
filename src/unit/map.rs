...
use crate::{
    ...
    unit::{description::UnitDescription, unit::Unit},
};
...
impl UnitMap {
    ...
    pub fn add(&self, key: impl Into<String>, description: UnitDescription) -> Option<LogReader> {
        let mut lock = self.units.write();
        match lock.entry(key.into()) {
            Entry::Occupied(_) => None,
            Entry::Vacant(vacant_entry) => {
                let unit = Unit::new(description);
                Some(vacant_entry.insert(unit).log_reader())
            }
        }
    }

    /// Start the unit under `key`, using its own description.
    ///
...
mod test {
    ...
    fn map(keys: &[&str]) -> Arc<UnitMap> {
        let map = UnitMap::new();
        for key in keys {
            map.add(*key, UnitDescription::noop());
        }
        map
    }
    ...
    async fn adding_over_a_name_replaces_what_was_there() {
        ...
        map.add("server", UnitDescription::noop());
        assert!(matches!(map.state("server"), Ok(RunnerState::Stopped)));
    }
}
